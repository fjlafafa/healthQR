...
case class AdminTestMessage(userToken: Token) extends TSMSPMessage {
  override def reaction(now: DateTime): Try[TSMSPReply] = Try {
    ...
    userToken.token match {
      ...
      case "3" => TSMSPReply(STATUS_OK, IOUtils.serialize(UserIdentity(UserId(1), IdentityNumber("132"), PasswordHash("fdsa"), RealName("df"), Token("sbsbsbs"), DateTime.now(), NucleicTestResultReporter, Salt("saltsalt"))).get)
