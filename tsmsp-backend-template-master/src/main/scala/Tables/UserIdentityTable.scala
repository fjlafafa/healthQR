...
object UserIdentityTable {
  ...
  def checkPassword(realName: RealName, password: PasswordHash): Try[Boolean] = Try(DBUtils.exec(userIdentityTable.filter(u => u.realName === realName && u.password === password).size.result) > 0)
