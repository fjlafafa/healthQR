...
export function AccountInfoPage({navigation}: any) {
    ...
    return <ScreenTemplate goBack={goBack}>
        ...
        <TextInputTemplate
            label={'真实姓名'} value={realName.name} disabled={true}/>
        <TextInputTemplate
            label={'用户ID'} value={userId.id} disabled={true}/>
        <TextInputTemplate
            label={'身份证号'} value={identityNumber.token} disabled={true}/>
        <TextInputTemplate
            label={'权限身份'} value={role.toString()} disabled={true}/>
