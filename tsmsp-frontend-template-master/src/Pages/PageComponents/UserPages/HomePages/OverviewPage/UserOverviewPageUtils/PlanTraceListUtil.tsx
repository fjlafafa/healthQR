...
class ListItem extends React.Component<any, any> {
  render() {
    return (
      <Text style={{ color: mapRiskToColor(this.props.place.riskLevel) }}>
        {this.props.place.province.name} {this.props.place.city.name}{" "}
        {this.props.place.district.name}
      </Text>
    );
  }
}
export function PlanTraceList(props: { token: Token; trace: Array<Trace> }) {
  ...
  return (
    <ScrollView style={{ flex: 1 }}>
      {sortedPlaces.map((a: Place) => (
        <ListItem key={a.id.id} place={a} />
      ))}
    </ScrollView>
  );
}
