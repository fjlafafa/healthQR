...
export function NucleicAcidView(props: { recentNucleicTestTime: DateClass }) {
  const millis = new Date().getTime() - props.recentNucleicTestTime.millis;
  const nucleicAcidColor =
    millis < 3 * DAY_MILLIS
      ? "goldenrod"
      : millis < 10 * DAY_MILLIS
      ? "forestgreen"
      : "grey";
  ...
  return (
    <Card
      style={{
        ...
        backgroundColor: nucleicAcidColor,
      }}
    >
      <Card.Content>
        <Text style={setting.text}>核酸检测结果</Text>
      </Card.Content>
      <Card.Content>
        <Text style={setting.text}>距离上次核酸检测 {nucleicAcidTime} 天</Text>
      </Card.Content>
    </Card>
  );
}
